 ...
 */
function log(color = "white", suffix = "") {
	/**
	 * @param {string} data
	 */
	return (...data) => {
		...
		let ms = time.getMilliseconds().toString().charAt(0);
		...
		if (process.stdout._type == "tty") process.stdout.write(
			`\r \x1b[${colors[color]};1m[${month}-${date} ${hours}:${minutes}:${seconds}.${ms}]\x1b[0m ` + suffix + " " + data.join(" ") + "\x1b[0m\n"
		); else if (process.stdout._type == "fs") process.stdout.write(
 ...
 * @param {string} data
 */
log.title = (color, data) => {
	titlecolor = colors[color];
	if (process.stdout._type == "tty")
		console.log(`\x1b[${colors[color]};7;1m ${data}    \x1b[0;${colors[color]}m\x1b[0m`);
	else {
		console.log(`<<<< ${data} >>>>`);
	}
}
log.end = () => {
	if (process.stdout._type == "tty") console.log(`\x1b[${titlecolor};7;1m \x1b[0;${titlecolor}m\x1b[0m`);
	else console.log(">>>> <<<<");
	titlecolor = "0";
}
log.clear = () => {
	if (process.stdout._type == "tty") process.stdout.write("\x1b[3J\x1b[2J\x1b[1;1H");
}
