...
export async function create(req: Request, res: Response) {
  try {
    ...
    const relation = new Relation()
      .setUsers(res.locals.user.id, req.params.user_id);
...
export async function getById(req: Request, res: Response) {
  try {
    ...
    const relation = await Relation.getByIds(res.locals.user.id, req.params.user_id);
    if (!relation) {
      return respond(res, 404, "NotFound");
    }

    return respond(res, 200, "Ok", relation.clean(res.locals.user.id));
  }
  catch (err) {
    log("red")((err as Error).message);
    return respond(res, 500, "InternalError");
...
export async function destroy(req: Request, res: Response) {
  try {
    const relation = new Relation().setUsers(res.locals.user.id, req.params.user_id);
    relation.delete();
  }
  catch (err) {
    log("red")((err as Error).message);
